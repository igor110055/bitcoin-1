...
function binanceConnection(symbol) {
	...
	socket.addEventListener("message", function (event) {
		...
		if (lastPrice > 10.0) {
			...
			$("#greetings").html('<font>'+tmp[0]+'</font><font style="color:#b2b5be; font-weight: 300;">.'+tmp[1]+'</font>');
			...
			$("#greetings").html(addCommas(lastPrice.toFixed(4)));
...
function exchangeRateReqListener () {
    ...
    $("#exchangeRate").text('K-Premium : ' + addCommas(basePrice));
	...
	socket.addEventListener("message", function (event) {
		...
		reader.readAsText(event.data);
  		reader.onload = function() {
    		...
    		$("#upbit").html('['+symbol+'] Upbit');
	        $("#upbitKrw").html(addCommas(json.trade_price));
	        if (curBtcusdt > 0.0) {
		        let binancekrw = curBtcusdt * curChangeRate;
		        $("#kpraemium").html((((json.trade_price - binancekrw) / binancekrw) * 100).toFixed(2) + '%');
	        }
			/***
			$("#greetings").html(addCommas(json.trade_price));
...
/**
function longShortRate(ws) {
    var socket = new SockJS(ws + '/websocket');
    stompClient = Stomp.over(socket);
    stompClient.connect({}, function () {
        stompClient.subscribe('/topic/longShortRate', function (greeting) {
            showLongShortRateMessage(JSON.parse(greeting.body).content);
        });
        stompClient.send("/app/longShortRate");
    });
}

function showLongShortRateMessage(message) {
	const obj = JSON.parse(message);
	var longRate = parseFloat(obj.longRate);
	var shortRate = parseFloat(obj.shortRate);
    ...
    if (longRate > shortRate) {
    	$(".card-header").css("background-color", '#26a69a');
    } else if (longRate < shortRate) {
    	$(".card-header").css("background-color", '#ef5350');
    } else {
    	$(".card-header").css("background-color", '#787b86');
    }
}
**/
